...
function ProjectDetail({
  ...
  return (
    <>
      <Topbar crumbs={['Workspace', 'Projects', project.name]} role={role} onCreate={onModalOpen} />
      <div style={{ overflow: 'auto', flex: 1, position: 'relative' }}>
        <div className="ph">
          <div className="ph-row">
            <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              <div style={{ width: 36, height: 36, borderRadius: 6, background: project.color || 'var(--terra)', display: 'grid', placeItems: 'center', color: 'white', fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: 13 }}>
                {project.key}
              </div>
              <div>
                ...
                <div className="ph-meta">
                  ...
                  {project.description && <span style={{ color: 'var(--ink-3)' }}>{project.description}</span>}
                </div>
              </div>
            </div>
            <div className="ph-actions">
              <button className="btn btn-accent" onClick={onModalOpen}><Ico.plus /> New task</button>
            </div>
          </div>
          <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 14, fontSize: 11, color: 'var(--ink-3)', fontFamily: 'var(--font-mono)' }}>
              ...
              <span><span style={{ display: 'inline-block', width: 6, height: 6, borderRadius: '50%', background: '#b48a2c', marginRight: 5 }} />{statusCounts.in_progress} In Progress</span>
              ...
              <span><span style={{ display: 'inline-block', width: 6, height: 6, borderRadius: '50%', background: '#5c7a5a', marginRight: 5 }} />{done} Done</span>
            </div>
          </div>
        </div>
        ...
        {view === 'list'
          ? <ProjectListView tasks={tasks} onDrawerOpen={onDrawerOpen} />
          : <ProjectBoardView tasks={tasks} onDrawerOpen={onDrawerOpen} />
        }
        ...
        {modal && (
          <NewTaskModal project={project} team={team} user={user} onClose={onModalClose} onCreated={onTaskCreated} />
        )}
      </div>
    </>
  );
...
function ProjectListView({ tasks = [], onDrawerOpen = () => {} }) {
  ...
  return (
    <div>
      ...
      {tasks.length === 0 && (
        <div style={{ padding: 40, textAlign: 'center', color: 'var(--ink-3)' }}>No tasks yet. Click "New task" to add the first one.</div>
      )}
    </div>
  );
}
...
function ProjectBoardView({ tasks = [], onDrawerOpen = () => {} }) {
  ...
  return (
    <div style={{ padding: 16, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, alignItems: 'start', overflowX: 'auto' }}>
      {cols.map(c => {
        const items = tasks.filter(t => t.status === c.key);
        return (
          <div key={c.key} style={{ background: 'var(--bg-elev)', borderRadius: 6, border: '1px solid var(--line)', minWidth: 180 }}>
            ...
            <div style={{ padding: 8, display: 'grid', gap: 8 }}>
              ...
              {items.length === 0 && (
                <div style={{ fontSize: 11, color: 'var(--ink-4)', padding: '6px 4px', textAlign: 'center' }}>Empty</div>
              )}
            </div>
          </div>
        );
...
function TaskDrawer({ task, role, team = [], onClose = () => {}, onTaskUpdated = () => {} }) {
  ...
  const [editAssignee, setEditAssignee] = React.useState(task.assignee_id || '');
  const [editDue, setEditDue]     = React.useState(task.due_date ? task.due_date.split('T')[0] : '');
  const [saveErr, setSaveErr]     = React.useState('');

  const updateStatus = async (newStatus) => {
    setSaving(true);
    try {
      const updated = await API.put(`/tasks/${t.id}`, { status: newStatus });
  ...
  const saveEdits = async () => {
    ...
    try {
      const updated = await API.put(`/tasks/${t.id}`, {
        title: editTitle,
        description: editDesc,
        assignee_id: editAssignee || null,
        due_date: editDue || null,
      });
      const assigneeName = team.find(m => String(m.id) === String(editAssignee))?.name || t.assignee_name;
      const merged = { ...t, ...updated, assignee_name: assigneeName };
      setT(merged);
      onTaskUpdated(merged);
      setEditing(false);
  ...
  return (
    <div className="drawer">
      <div className="drawer-h">
        ...
        <span style={{ marginLeft: 4, fontSize: 11, color: 'var(--ink-3)' }}>{t.project_key}</span>
        ...
        {editing ? (
          <div style={{ display: 'grid', gap: 12 }}>
            ...
            <div className="field">
              ...
              <textarea className="textarea" rows={3} value={editDesc} onChange={e => setEditDesc(e.target.value)} placeholder="Add context…" />
            </div>
            ...
            {saveErr && <div style={{ color: 'var(--rust)', fontSize: 12 }}>{saveErr}</div>}
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {['todo', 'in_progress', 'review', 'done'].map(s => (
                <button key={s}
                  className={`btn btn-sm ${t.status === s ? 'btn-primary' : ''}`}
                  disabled={saving}
...
// NewTaskModal — works from project detail (project pre-selected) or global (shows project picker)
function NewTaskModal({ project = null, projects = [], team = [], user = {}, onClose = () => {}, onCreated = () => {} }) {
  const [selectedProjectId, setSelectedProjectId] = React.useState(project ? project.id : '');
  const [title, setTitle]       = React.useState('');
  const [desc, setDesc]         = React.useState('');
  const [status, setStatus]     = React.useState('todo');
  const [priority, setPrio]     = React.useState('medium');
  const [assignee, setAssignee] = React.useState(user.id ? String(user.id) : '');
  const [due, setDue]           = React.useState('');
  const [err, setErr]           = React.useState('');
  const [loading, setLoading]   = React.useState(false);

  const resolvedProject = project || projects.find(p => String(p.id) === String(selectedProjectId));

  const submit = async () => {
    if (!title.trim()) { setErr('Title is required'); return; }
    if (!selectedProjectId) { setErr('Select a project'); return; }
    ...
    try {
      ...
      // Attach project info for immediate UI update
      const proj = resolvedProject;
      onCreated({ ...task, project_name: proj?.name, project_key: proj?.key, project_color: proj?.color,
        assignee_name: team.find(m => String(m.id) === String(assignee))?.name || null });
      onClose();
    } catch (e) {
      setErr(e.message);
    } finally {
  ...
  const projectLabel = project ? project.name : (resolvedProject ? resolvedProject.name : null);
    ...
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="modal-h">
          ...
          {projectLabel && <span style={{ marginLeft: 8, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--ink-3)' }}>in {projectLabel}</span>}
          <button className="icon-btn" style={{ marginLeft: 'auto' }} onClick={onClose}><Ico.close /></button>
        </div>
        <div className="modal-b">
          {!project && (
            <div className="field">
              <label className="field-l">Project <span style={{ color: 'var(--rust)' }}>*</span></label>
          ...
          <div className="field">
            ...
            <input className="input" placeholder="What needs to get done?" value={title}
              onChange={e => setTitle(e.target.value)} autoFocus
              onKeyDown={e => e.key === 'Enter' && !e.shiftKey && submit()} />
          </div>
          <div className="field">
            <label className="field-l">Description</label>
            <textarea className="textarea" placeholder="Add context, acceptance criteria…" value={desc} onChange={e => setDesc(e.target.value)} />
          </div>
          <div className="field-row">
            <div className="field">
